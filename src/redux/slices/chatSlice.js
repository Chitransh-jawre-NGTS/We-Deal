...
export const sendMessage = createAsyncThunk(
  "chat/sendMessage",
  async ({ chatId, text, sender }, { rejectWithValue }) => {
    try {
      const response = await chatApi.sendMessage(chatId, { text, sender });
      // return the message object, or fallback to the sent payload
      return { chatId, message: response.data || { chatId, text, sender } };
    } catch (error) {
      return rejectWithValue(error.response?.data || error.message);
    }
  }
...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    // ✅ Add message from socket or optimistic UI
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(findOrCreateChat.fulfilled, (state, action) => {
        state.loading = false;
        const chatExists = state.chats.find((c) => c.id === action.payload.id);

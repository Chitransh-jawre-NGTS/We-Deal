...
const ImageUpload = () => {
  ...
  const category = location.state?.category || "general";
  let coords = location.state?.coords || null;
  const planType = location.state?.planType || "base"; // ✅ get planType from previous page
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("planType", planType); // ✅ send planType to backend
      images.forEach((img) => formData.append("images", img));
      ...
      navigate("/"); // or navigate("/my-ads") if you have a listing page
    } catch (err) {
      if (err.response?.status === 403 && err.response.data.redirectTo) {
        // redirect to billing page if user has no remaining ads
        window.location.href = err.response.data.redirectTo;
        return;
      }

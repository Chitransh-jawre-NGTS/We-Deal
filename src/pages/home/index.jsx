          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            {featuredItems.map((item, idx) => (
              <React.Fragment key={idx}>
                <div className="relative bg-white border border-purple-500 shadow-md overflow-hidden cursor-pointer transition hover:shadow-xl hover:scale-105 transform">
                  {item.featured && (
                    <span className="absolute top-2 left-2 bg-yellow-400 text-black text-xs font-bold px-2 py-1 rounded">
                      Featured
                    </span>
                  )}
                  <FaHeart className="absolute top-2 right-2 text-white border-gray-500 text-lg cursor-pointer" />
                  <img
                    src={item.image}
                    alt={item.title}
            ...
            <section className="w-full bg-black py-8 md:py-12">
                <div className="max-w-6xl mx-auto px-4">
                    <div className="relative w-full aspect-video rounded-xl overflow-hidden shadow-lg">
                        {/* Example Google Ads / YouTube video embed */}
                        <iframe
